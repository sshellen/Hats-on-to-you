...
class HatDrop extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      showSettings: false,
      speed: 5,
      speedSetting: "gradual"
    };
  }

  moveHatDrop = () => {
    hatDropInterval = setTimeout(() => {
      hatDropLeft += speed;
      hatDrop.style.left = hatDropLeft + "px";
      if (hatDropLeft > this.props.rightWall) {
        hatDropLeft = -60;
      }
      window.clearTimeout(hatDropInterval);
      hatDropFrame = window.requestAnimationFrame(this.moveHatDrop);
    }, 60);
  };
  ...
  componentWillUnmount() {
    window.clearTimeout(hatDropInterval);
    window.cancelAnimationFrame(hatDropFrame);
    hatDropLeft = 0;
